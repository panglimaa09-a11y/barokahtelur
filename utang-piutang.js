(function(){
  ...
  async function pay(id){try{const r=rows.find(x=>x.id===id);if(!r)return;const left=balance(r);if(left<=0){alert('Catatan ini sudah lunas.');return;}const amount=Number(prompt('Masukkan nominal pembayaran:\nSisa '+fmt(left),'0'));if(!Number.isFinite(amount)||amount<=0||amount>left){alert('Nominal pembayaran tidak valid.');return;}const note=prompt('Keterangan pembayaran (opsional):','')||'';const u=await user();const {data:p,error:pe}=await SB().from('debt_payments').insert({debt_id:id,user_id:u.id,payment_date:today(),amount,note}).select('*').single();if(pe)throw pe;const {data:all,error:ae}=await SB().from('debt_payments').select('amount').eq('debt_id',id).eq('user_id',u.id);if(ae)throw ae;const paid=(all||[]).reduce((s,x)=>s+Number(x.amount||0),0);const {error:ue}=await SB().from('debts_receivables').update({paid_amount:paid}).eq('id',id).eq('user_id',u.id);if(ue){await SB().from('debt_payments').delete().eq('id',p.id).eq('user_id',u.id);throw ue;}await load();alert('Pembayaran berhasil dicatat.');}catch(e){alert('Gagal mencatat pembayaran: '+(e.message||e));}}
  ...
  function render(){
    ...
    box.innerHTML='<table><thead><tr><th>Nama</th><th>Tanggal</th><th>Referensi</th><th>Total</th><th>Dibayar</th><th>Sisa</th><th>Jatuh Tempo</th><th>Status</th><th>Aksi</th></tr></thead><tbody>'+list.map(r=>{const st=status(r);return '<tr><td><strong>'+esc(r.party_name)+'</strong><span class="sub">'+esc(r.phone||r.party_type)+'</span></td><td>'+esc(r.debt_date)+'</td><td>'+esc(r.reference_no||'-')+'</td><td>'+fmt(r.total_amount)+'</td><td>'+fmt(r.paid_amount)+'</td><td><strong>'+fmt(balance(r))+'</strong></td><td>'+esc(r.due_date||'-')+'</td><td><span class="debt-status '+st[1]+'">'+st[0]+'</span></td><td><button class="debt-pay" data-pay="'+r.id+'">Bayar</button> <button class="debt-del" data-del="'+r.id+'">Hapus</button></td></tr>';}).join('')+'</tbody></table>';
