(function(){
  ...
  function css(){
    ...
    s.textContent=`
      ...
      #page-debt .debt-table{overflow:auto}#page-debt table{min-width:980px;width:100%;border-collapse:collapse}#page-debt th,#page-debt td{padding:12px;border-bottom:1px solid #edf0ed;text-align:left;font-size:12px}#page-debt th{font-size:10px;text-transform:uppercase;color:#6d776f}.debt-status{display:inline-block;padding:5px 8px;border-radius:999px;font-size:10px;font-weight:900}.debt-status.lunas{background:#edf8f1;color:#087344}.debt-status.sebagian{background:#fff7d5;color:#725900}.debt-status.belum{background:#fff0ee;color:#a72e28}.debt-pay{border:0;border-radius:9px;background:#0d5b45;color:#fff;padding:8px 10px;font-weight:800;font-size:11px}.debt-del{border:1px solid #f2cdc9;border-radius:9px;background:#fff0ee;color:#bd4037;padding:7px 9px;font-weight:800;font-size:11px}.debt-toolbar{display:flex;justify-content:space-between;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:12px}.debt-toolbar input{height:38px;border:1px solid #d6ded8;border-radius:9px;padding:0 10px;min-width:220px}.debt-empty{text-align:center;color:#6d776f;padding:28px}.debt-help{font-size:11px;color:#6d776f;margin:4px 0 14px}
      @media(max-width:850px){#page-debt .debt-summary{grid-template-columns:1fr 1fr}}@media(max-width:600px){#page-debt .debt-summary{grid-template-columns:1fr}.debt-form{grid-template-columns:1fr!important}.debt-field.full{grid-column:auto}.debt-toolbar input{width:100%;min-width:0}}
  ...
  function page(){
    ...
    el.innerHTML=`<div class="hero"><div class="eyebrow">KEUANGAN</div><h1>Utang Piutang</h1><p>Kelola utang kepada supplier dan piutang pelanggan secara terpisah dari Riwayat Transaksi.</p></div>
      ...
      <div class="debt-panel"><div class="debt-tabs"><button data-debt-tab="piutang" class="active">Piutang Pelanggan</button><button data-debt-tab="utang">Utang Supplier</button></div><div class="debt-toolbar"><strong id="debtListTitle">Daftar Piutang</strong><div style="display:flex;gap:8px;flex-wrap:wrap"><input id="debtSearch" placeholder="Cari nama / referensi..."><button class="btn ghost" id="debtPrint">🖨️ Cetak</button></div></div><div class="debt-table" id="debtTable"></div></div>`;
    wrap.appendChild(el);
  ...
  function nav(){
    ...
    const b=document.createElement('button');b.id='debtNavBtn';b.textContent='💰 Utang Piutang';b.addEventListener('click',()=>{if(typeof showPage==='function')showPage('debt');else document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById('page-debt').classList.add('active');load();});n.appendChild(b);
  }
  ...
  function render(){
    ...
    box.innerHTML='<table><thead><tr><th>Nama</th><th>Tanggal</th><th>Referensi</th><th>Total</th><th>Dibayar</th><th>Sisa</th><th>Jatuh Tempo</th><th>Status</th><th>Aksi</th></tr></thead><tbody>'+list.map(r=>{const st=status(r);return '<tr><td><strong>'+esc(r.party_name)+'</strong><span class="sub">'+esc(r.phone||r.party_type)+'</span></td><td>'+esc(r.debt_date)+'</td><td>'+esc(r.reference_no||'-')+'</td><td>'+fmt(r.total_amount)+'</td><td>'+fmt(r.paid_amount)+'</td><td><strong>'+fmt(balance(r))+'</strong></td><td>'+esc(r.due_date||'-')+'</td><td><span class="debt-status '+st[1]+'">'+st[0]+'</span></td><td><button class="debt-pay" data-pay="'+r.id+'">'+(st[0]==='Lunas'?'Lunas':'Bayar')+'</button> <button class="debt-del" data-del="'+r.id+'">Hapus</button></td></tr>';}).join('')+'</tbody></table>';
