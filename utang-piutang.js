(function(){
  ...
  function openEdit(id){
    ...
    document.getElementById('editDebtQty').value=Number(r.quantity||1);
    document.getElementById('editDebtUnit').value=r.unit||'Paket';
    document.getElementById('editDebtNote').value=r.note||'';
    const m=document.getElementById('debtEditModal');m.classList.add('open');m.setAttribute('aria-hidden','false');setTimeout(()=>document.getElementById('editDebtParty').focus(),30);
  }
  ...
  function patchShowPage(){
    ...
    if(typeof current!=='function'||current.__barokahDebtPatched)return;
    const wrapped=function(name){
      const result=current.apply(this,arguments);
      if(name==='debt')setTimeout(()=>load(),0);
      return result;
    };
    wrapped.__barokahDebtPatched=true;
  ...
  function init(){
    css();page();nav();
    patchShowPage();
    if(document.getElementById('page-debt')?.classList.contains('active'))setTimeout(()=>load(),0);
    setTimeout(patchShowPage,500);
  }
