...
create policy transactions_select_shared on public.transactions for select to authenticated using (true);
create policy transactions_insert_shared on public.transactions for insert to authenticated with check (auth.uid() = user_id);
create policy transactions_update_shared on public.transactions for update to authenticated using (true) with check (auth.uid() = user_id);
create policy transactions_delete_shared on public.transactions for delete to authenticated using (true);
...
create policy stock_select_shared on public.stock_movements for select to authenticated using (true);
create policy stock_insert_shared on public.stock_movements for insert to authenticated with check (auth.uid() = user_id);
create policy stock_delete_shared on public.stock_movements for delete to authenticated using (true);

-- Avoid recursive RLS evaluation on profiles.
...
-- Keep the profile synchronized with Supabase Auth.
...
begin
  ...
  values (
    ...
    new.email,
    case when coalesce(new.raw_user_meta_data->>'role','') = 'employee' then 'employee' else 'owner' end
  )
