...
async function syncShared(){
  ...
  // Never destroy the user's local recovery copy just because the shared
  // query is temporarily empty (for example before RLS is migrated).
  if(rows.length){
    try{localStorage.setItem('barokah_telur_shared_cache_v1',JSON.stringify(window.state));}catch(e){}
  }
