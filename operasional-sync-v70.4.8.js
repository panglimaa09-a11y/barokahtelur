...
function setStatus(text){
  ...
  if(box && !box.querySelector('table')) box.innerHTML='<div class="op-empty">'+esc(text)+'</div>';
}
...
async function sync(){
  ...
  const sb=SB();
  if(!sb){ setStatus('Menunggu koneksi database...'); return; }
  running=true;
  try{
    const u=await getSessionUser();
    if(!u){ setStatus('Menunggu sesi login...'); return; }

    // RLS sudah membatasi data berdasarkan user login. Filter user_id tetap dipakai
    // sebagai pengaman tambahan agar Dashboard dan Operasional memakai owner yang sama.
    const result=await sb.from('operational_transactions')
      .select('*')
      .eq('user_id',u.id)
    ...
    if(!list.length){
      box.innerHTML='<div class="op-empty">Belum ada transaksi operasional.</div>';
    }else{
      box.innerHTML='<table><thead><tr><th>No. Transaksi</th><th>Tanggal</th><th>Jenis</th><th>Kategori</th><th>Keterangan</th><th>Nominal</th><th>Aksi</th></tr></thead><tbody>'+list.map(r=>
        '<tr><td class="op-ref">'+esc(r.reference_no||'-')+'</td><td>'+esc(r.transaction_date||'-')+'</td><td><span class="'+(r.kind==='pemasukan'?'op-in':'op-out')+'">'+(r.kind==='pemasukan'?'Pemasukan':'Pengeluaran')+'</span></td><td>'+esc(r.category||'-')+'</td><td><strong>'+esc(r.description||'-')+'</strong><span class="sub">'+esc(r.note||'')+'</span></td><td class="'+(r.kind==='pemasukan'?'op-in':'op-out')+'">'+fmt(r.amount)+'</td><td class="op-actions-cell"><button class="op-del" data-op-del="'+esc(r.id)+'">Hapus</button><button class="op-nota" data-op-nota="'+esc(r.id)+'">🧾 Nota</button></td></tr>'
      ).join('')+'</tbody></table>';
    }
    window.dispatchEvent(new CustomEvent('barokah:operational-synced',{detail:{rows,userId:u.id}}));
  }catch(e){
    console.error('Operasional sync gagal:',e);
    setStatus('Gagal memuat riwayat operasional: '+(e.message||e));
...
function start(){
  ...
  if(SB()&&SB().auth) SB().auth.onAuthStateChange((event)=>{
    if(event==='SIGNED_IN'||event==='TOKEN_REFRESHED'||event==='USER_UPDATED'||event==='SIGNED_OUT') setTimeout(sync,100);
  });
}
