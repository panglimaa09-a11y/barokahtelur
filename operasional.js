(function(){
  ...
  function nav(){
    ...
    const b=document.createElement('button');b.id='opNavBtn';b.textContent='💸 Operasional';b.addEventListener('click',()=>{if(typeof showPage==='function')showPage('operational');else document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById('page-operational').classList.add('active');load();});n.appendChild(b);
  }
  ...
  async function dashboard(){
    ...
    try{const u=await user();const [opRes,debtRes]=await Promise.all([
      ...
      host.innerHTML=`<div class="op-dashboard"><div class="eyebrow">RINGKASAN TAMBAHAN</div><h2 style="margin:5px 0 0">💰 Utang Piutang & 💸 Operasional</h2><div class="op-debt"><div class="op-debt-grid"><div class="op-debt-card piutang"><small>Total Piutang</small><strong>${fmt(piu)}</strong></div><div class="op-debt-card utang"><small>Total Utang</small><strong>${fmt(utg)}</strong></div><div class="op-debt-card open"><small>Belum Lunas</small><strong>${open} catatan</strong></div><div class="op-debt-card paid"><small>Saldo Operasional</small><strong>${fmt(ins-outs)}</strong></div></div><div class="op-dashboard-actions"><button class="btn primary" type="button" id="dashboardDebtBtn">💰 Buka Utang Piutang</button><button class="btn ghost" type="button" id="dashboardOpBtn">💸 Buka Operasional</button></div></div><div class="op-dashboard-grid"><div class="op-dashboard-item"><small>Pemasukan Operasional</small><strong style="color:#087344">${fmt(ins)}</strong></div><div class="op-dashboard-item"><small>Pengeluaran Operasional</small><strong style="color:#b42318">${fmt(outs)}</strong></div></div></div>`;
    ...
    }catch(e){console.warn('Dashboard tambahan belum siap:',e.message||e);host.innerHTML='<div class="op-dashboard"><div class="eyebrow">RINGKASAN TAMBAHAN</div><h2 style="margin:5px 0">💰 Utang Piutang & 💸 Operasional</h2><p style="color:#6d776f;font-size:12px">Data akan muncul setelah sesi login dan migration operasional siap.</p></div>';}
  }
